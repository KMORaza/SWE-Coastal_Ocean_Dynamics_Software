...
public class ShallowWaterSimulation extends Application {
    ...
    private double WIND = 0.0; // Wind stress (N/m²)
    ...
    public void start(Stage primaryStage) {
        ...
        primaryStage.setTitle("SWE - Coastal Ocean Dynamics Simulator");
        ...
        new AnimationTimer() {
            ...
            public void handle(long now) {
                if (running && (now - lastUpdate) >= 16_000_000) { // ~60 FPS
                    updateSimulation();
                    updateParticles();
                    drawSimulation();
                    updateChart();
    ...
    private void initializeFields() {
        ...
        // Initialize with coastal bathymetry (shallowing toward one side)
        for (int i = 0; i < GRID_SIZE; i++) {
            for (int j = 0; j < GRID_SIZE; j++) {
                h[i][j] = 10.0;
                u[i][j] = 0.0;
                v[i][j] = 0.0;
                b[i][j] = 8.0 * (1.0 - (double)i / GRID_SIZE); // Linear slope
            }
        }
        particles.clear();
        addParticles();
    ...
    private TabPane createControls() {
        ...
        // HBox for horizontal button layout, shifted left
        HBox buttonBox = new HBox(10);
        buttonBox.setAlignment(Pos.CENTER_LEFT); // Changed from Pos.CENTER to Pos.CENTER_LEFT
        buttonBox.getChildren().addAll(startButton, resetButton, tsunamiButton, tideButton);
        ...
        // Wind
        windCheck = new CheckBox("Enable Wind Stress");
        ...
        visControls.getChildren().addAll(
                ...
                visualizationMode, colormapMode, particleCheck, legendText, exportButton
        );
        ...
        helpControls.getChildren().addAll(
                new Label("About the app"),
                new Text("This simulator models coastal ocean dynamics.\n" +
                        "- To visualize tsunamis: click 'Simulate Tsunami' for a wave pulse.\n" +
                        "- To visualize tides: Click 'Simulate Tide' for periodic forcing.\n" +
                        "- Storm Surges: Enable wind stress.\n" +
    ...
    private void updateSimulation() {
        ...
        // Apply tidal forcing if active
        if (tideActive) {
            double time = (System.nanoTime() - tideStartTime) / 1e9;
            for (int i = 0; i < GRID_SIZE; i++) {
                h[i][0] = 10.0 + 5.0 * Math.sin(2 * Math.PI * time / 3600);
            }
        }
        ...
        for (int i = 1; i < GRID_SIZE - 1; i++) {
            for (int j = 1; j < GRID_SIZE - 1; j++) {
                ...
                // Momentum equation (x)
                double u_adv = u[i][j] * (u[i+1][j] - u[i-1][j]) / (2 * DX) +
                        v[i][j] * (u[i][j+1] - u[i][j-1]) / (2 * DX);
                double h_grad_x = G * (h[i+1][j] - h[i-1][j]) / (2 * DX);
                double b_grad_x = G * (b[i+1][j] - b[i-1][j]) / (2 * DX);
                uNew[i][j] = u[i][j] - DT * (u_adv + h_grad_x + b_grad_x - wind / (h[i][j] + 1e-6)) +
                        DT * f * v[i][j] - DT * k * u[i][j];

                // Momentum equation (y)
                double v_adv = u[i][j] * (v[i+1][j] - v[i-1][j]) / (2 * DX) +
                        v[i][j] * (v[i][j+1] - v[i][j-1]) / (2 * DX);
                double h_grad_y = G * (h[i][j+1] - h[i][j-1]) / (2 * DX);
                double b_grad_y = G * (b[i][j+1] - b[i][j-1]) / (2 * DX);
                vNew[i][j] = v[i][j] - DT * (v_adv + h_grad_y + b_grad_y) -
    ...
    private void drawSimulation() {
        ...
        // Compute min/max with tighter range for visibility
        ...
        for (int i = 0; i < GRID_SIZE; i++) {
            for (int j = 0; j < GRID_SIZE; j++) {
                double value = mode.equals("Velocity Magnitude") ?
                        ...
                        mode.equals("Bathymetry") ? b[i][j] : h[i][j];
